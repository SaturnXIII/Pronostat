...
chrome_binary_path = r'C:\Users\utilisateur\Chemin\vers\chrome-win64\chrome.exe'
...
options.add_argument("--start-maximized")  # Pour maximiser la fenêtre (équivalent à la taille de l'appareil mobile)
options.add_argument("--disable-infobars")  # Pour masquer la barre d'info

# Exemple de configuration pour émuler un iPhone X
...
options.add_argument("--window-size=375,812")  # Taille d'écran de l'iPhone X
...
url2 = 'https://url_lycee/pronote/eleve.html'
...
# Chemin vers le fichier .exe que vous souhaitez exécuter
chemin_vers_exe = "../files/click.exe"
time.sleep(5)
# Lancer le programme .exe
subprocess.call(chemin_vers_exe)

# Récupérer les informations spécifiques de l'élément avec l'ID "id_79"
id_79_element = driver.find_element(By.ID, "id_62")
id_79_info = id_79_element.text
photo = driver.find_element(By.ID, "id_61")
...
with open("../friendlist/dexemple.txt", "a") as friendlist_file:
